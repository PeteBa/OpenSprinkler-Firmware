...
#define IFTTT_EVENT_MAXNUM         6    // Maximum number of unique IFTTT endpoints
...
void push_ifttt_message(ulong timestamp, notification_t type, push_msg_t * m) {
  ...
  // File contains event endpoints in json format: "events":["event1","event2","event3","event4","event5","event6"]
...
void post_msg(const char * server, int port, const char * path, const char * postval)
{
  ...
  if (!ether.dnsLookup(server, true, 5000)) {
  ...
  while ((millis() - start < 5000) && (post_completed == false))
